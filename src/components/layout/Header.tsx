...
export default function Header({ title }: { title: string }) {
  ...
  return <div className="p-4 border-b space-y-2"><div className="flex items-center justify-between"><div><h1 className="text-xl font-bold">{title}</h1><p className="text-sm text-muted-foreground">Signed in as {user?.username}</p></div><div className="flex gap-2"><DarkModeToggle /><Button variant="destructive" onClick={logout}>Logout</Button></div></div><div className="flex flex-wrap gap-4 text-sm">{user?.role === "student" ? getStudentNavItems(enabledModules).map((item) => <Link key={item.href} to={item.href} className="underline">{item.label}</Link>) : <><Link to="/admin" className="underline">Dashboard</Link><Link to="/admin/students" className="underline">Students</Link><Link to="/admin/materials" className="underline">Materials</Link><Link to="/admin/content" className="underline">Content</Link><Link to="/admin/analytics" className="underline">Analytics</Link></>}</div></div>;
}
